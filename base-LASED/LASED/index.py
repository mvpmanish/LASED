...
def index(i, j, n):
    ...
    Parameters:
        i (State/int): To get the row number of the element to be selected
        j (State/int): To get the column number of the element to be selected
    ...
    Example:
        To get the index of rho_33 in a 4-level system call index(three, three, 4) OR index(3, 3, 4) where "four" is the variable which stores the State object labelled 3.
    """
    #try and except block to catch if user uses int-indexing or State-indexing
    try:
        i_index = i.label
        j_index = j.label
    except: # An int-based index
        i_index = i
        j_index = j

    return ((i_index-1)*n+j_index)-1  # index starts at 0
